...
void main(void)
{
  unsigned char dco_1mhz_init;
  unsigned char dco_1mhz_final;
  WDTCTL = WDTPW + WDTHOLD;                 // Stop WDT
  for (i = 0; i < 0xfffe; i++);             // Delay for XTAL stabilization

  dco_1mhz_init = CALBC1_1MHZ;

  P6OUT = 0x00;                             // Clear P6 output latches
  P6DIR  = 0x07;                            // P6.0, 6.1, 6.2 out
  ...
  *Flash_ptrA = 0x00;                       // Dummy write to erase Flash seg A
  ...
  dco_1mhz_final = CALBC1_1MHZ;
  if(dco_1mhz_init == 0xff){
    P6OUT |= 0x02;
  }
  if(dco_1mhz_init != dco_1mhz_final){
    P6OUT |= 0x04;
  }
  ...
  while (1)
  {
    ...
    //TODO: latch out the settings from calibration to P6
    //something like 
    //P6OUT ^= 0x01
    //                0b1111 1101
    //P6OUT = (P6OUT & 0xfd) | (( CALBC1_1MHZ >> bit_num & 0x01) << 1)
